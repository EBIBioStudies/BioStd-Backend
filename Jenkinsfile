pipeline {
  ...
  stages {
    ...
    stage('Run') {
      steps {
        // sh 'pkill -f biostudy'
        sh "nohup ${env.JAVA_HOME}/bin/java -Xdebug -Xrunjdwp:transport=dt_socket,server=y,address=8001,suspend=n -jar /home/jhoan/EBI/deployments/BackendWebApp/biostudy-\$(date +'%Y%m%d').jar --biostudy.baseDir=\"/home/jhoan/EBI/nfs/local/input/01\" --biostudy.environment=DEV --eutoxrisk-file-validator.enabled=true --spring.mail.host=\"smtp.ebi.ac.uk\""
      }
    }
  }
}
