...
import static uk.ac.ebi.biostd.treelog.LogNode.Level.SUCCESS;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.PublicTag;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.SubmissionFilesDir;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getDefaultSubmissionAccPrefix;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getDefaultSubmissionAccSuffix;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getInstanceId;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getPublicFTPPath;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getSeqNumber;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getServiceManager;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getSubmissionFilesPath;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getSubmissionHistoryPath;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getSubmissionPath;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getSubmissionPublicFTPPath;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getSubmissionRelativePath;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getSubmissionsTransactionPath;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.getSubscriptionEmailSubject;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.rwxrwx___;
import static uk.ac.ebi.biostd.webapp.server.config.BackendConfig.rwxrwxr_x;
import static uk.ac.ebi.biostd.webapp.server.mng.SubmissionManager.Operation.CREATE;
import static uk.ac.ebi.biostd.webapp.server.mng.SubmissionManager.Operation.CREATEOVERRIDE;
import static uk.ac.ebi.biostd.webapp.server.mng.SubmissionManager.Operation.CREATEUPDATE;
import static uk.ac.ebi.biostd.webapp.server.mng.SubmissionManager.Operation.OVERRIDE;
import static uk.ac.ebi.biostd.webapp.server.mng.SubmissionManager.Operation.UPDATE;
...
public class JPASubmissionManager implements SubmissionManager {
    ...
    private final SubmissionQueryHelper queryHelper = new SubmissionQueryHelper();
    ...
    public LogNode deleteSubmissionByAccession(String acc, boolean toHistory, User usr) {
        ...
        try {
            ...
            if (!getServiceManager().getSecurityManager().mayUserDeleteSubmission(sbm, usr)) {
                gln.log(Level.ERROR, "User has no permission to delete this submission");
                return gln;
            }

            origDir = getSubmissionPath(sbm);

            histDir = getSubmissionHistoryPath(sbm);

            if (toHistory && Files.exists(origDir)) {
                histDirTmp = histDir.resolveSibling(histDir.getFileName() + "#tmp");

                try {
                    // trying to move submission directory to the history dir
                    fileMngr.moveDirectory(origDir, histDirTmp);

                    try {
                        //to provide access to the submission before the commit
                        Files.createSymbolicLink(origDir, histDirTmp);
                        dirOp = SubmissionDirState.LINKED;
                    } catch (Exception ex2) {
                        //if we can't make a symbolic link (FAT?) let's return the directory back
                ...
                if (dirOp == SubmissionDirState.HOME) {
                    try {
                        ...
                        // Bad. We have to break the operation
        ...
        if (trnOk && getPublicFTPPath() != null) {
            Path ftpPath = getSubmissionPublicFTPPath(sbm);
    ...
    public Submission getSubmissionsByAccession(String acc) {
        return queryHelper.getSubmissionsByAccession(acc);
    }
    ...
    private boolean checkAccNoPfxSfx(SubmissionInfo si) {
        ...
        if (si.getAccNoPrefix() == null && si.getAccNoSuffix() == null) {
            ...
            if (submission.getAccNo() == null) {
                si.setAccNoPrefix(getDefaultSubmissionAccPrefix());
                si.setAccNoSuffix(getDefaultSubmissionAccSuffix());
            }
        }
    ...
    private SecurityManager securityManager() {
        return getServiceManager().getSecurityManager();
    }
    ...
    private FileManager fileManager() {
        return getServiceManager().getFileManager();
    }

    private SubmissionReport createSubmissionUnsafe(
            byte[] data,
            DataFormat type,
            String charset,
            Operation opt,
            ...
            boolean ignoreFileAbs) {

        ErrorCounter errorCounter = new ErrorCounterImpl();

        SubmissionReport report = new SubmissionReport();
        SimpleLogNode gln = new SimpleLogNode(SUCCESS, opt.name() + " submission(s) from " + type.name() + " source",
                errorCounter);
        report.setLog(gln);

        if (opt == CREATE && !securityManager().mayUserCreateSubmission(usr)) {
        ...
        Path trnPath = getSubmissionsTransactionPath().resolve(getInstanceId() + "#" + getSeqNumber());
        boolean submOk = true;
        ...
        try {
            ...
            for (SubmissionInfo si : doc.getSubmissions()) {
                ...
                if (opt == UPDATE || opt == CREATEUPDATE || opt == OVERRIDE || opt == CREATEOVERRIDE) {
                    if ((si.getAccNoPrefix() != null || si.getAccNoSuffix() != null || submission.getAccNo() == null)
                        ...
                        si.getLogNode().log(Level.ERROR,
                                "Submission must have accession number for " + opt.name() + " operation");
                        submOk = false;
                        continue;
                    }
                    ...
                    if (oldSbm == null) {
                        if (opt == UPDATE || opt == OVERRIDE) {
                            si.getLogNode().log(Level.ERROR,
                                    "Submission '" + submission.getAccNo() + "' doesn't exist and can't be updated");
                            submOk = false;
                            continue;
                        } else {
                            if (!getServiceManager().getSecurityManager().mayUserCreateSubmission(usr)) {
                        ...
                        if (!getServiceManager().getSecurityManager()
                ...
                if (pAccL != null && pAccL.size() != 0) {
                    ...
                    for (String pAcc : pAccL) {
                        ...
                        if (!getServiceManager().getSecurityManager().mayUserAttachToSubmission(s, usr)) {
                            si.getLogNode().log(Level.ERROR, "User has no permission to attach to submission: " + pAcc);
                            submOk = false;
                            continue;
                        }

                        Match mtch = AccNoMatcher.match(si, s);
            ...
            for (SubmissionInfo si : doc.getSubmissions()) {
                ...
                report.addSubmissionMapping(sMap);

                if (!validateOnly && (si.getAccNoPrefix() != null || si.getAccNoSuffix() != null
                        || subm.getAccNo() == null)) {
                    while (true) {
                        try {
                            String newAcc = getServiceManager().getAccessionManager()
                    ...
                    for (SectionOccurrence seco : si.getGlobalSections()) {
                        if (!validateOnly && (seco.getPrefix() != null || seco.getSuffix() != null
                            ...
                            while (true) {
                                String newAcc = null;
                                try {
                                    newAcc = getServiceManager().getAccessionManager()
                ...
                if (si.getOriginalSubmission() != null) {
                    if (opt == OVERRIDE || opt == CREATEOVERRIDE) {
            ...
            if (!prepareFileTransaction(fileMngr, trans, doc.getSubmissions(), trnPath, opt)) {
            ...
            try {
                ...
                if (!submOk || !submComplete) {
                    ...
                    try {
                        ...
                        if (trans != null) {
                            try {
                                commitFileTransaction(fileMngr, trans, trnPath, opt);
    ...
    private void copyToPublicFTP(FileManager fileMngr, List<SubmissionInfo> list, LogNode gln) {
        for (SubmissionInfo si : list) {
            Path sourceDir = getSubmissionFilesPath(si.getSubmission());
            Path targetDir = getSubmissionPublicFTPPath(si.getSubmission());
            ...
            try {
                ...
                if (Files.exists(sourceDir) && getServiceManager().getSecurityManager()
    ...
    private void commitFileTransaction(FileManager fileMngr, List<FileTransactionUnit> trans, Path trnPath,
        ...
        for (FileTransactionUnit ftu : trans) {
            ...
            if (ftu.state == SubmissionDirState.COPIED) {
                if (op == OVERRIDE || op == CREATEOVERRIDE) {
                    fileMngr.deleteDirectory(ftu.historyPathTmp);
                } else {
    ...
    private boolean prepareFileTransaction(FileManager fileMngr, List<FileTransactionUnit> trans,
        ...
        for (SubmissionInfo si : subs) {
            ...
            Path origDir = getSubmissionPath(si.getSubmission());
            ...
            si.getSubmission().setRelPath(getSubmissionRelativePath(si.getSubmission()));

            ftu.state = SubmissionDirState.ABSENT;

            if (si.getOriginalSubmission() != null) {
                Path histDir = getSubmissionHistoryPath(si.getOriginalSubmission());
            ...
            Path sbmFilesPath = trnSbmPath.resolve(SubmissionFilesDir);
    ...
    public LogNode tranklucateSubmissionByAccessionPattern(String accPfx, User usr) {
        SimpleLogNode gln = new SimpleLogNode(SUCCESS, "Tranklucating submissions by pattern '" + accPfx + "'",
                null);
    ...
    private LogNode tranklucateSubmissionByAccession(String acc, User usr, LogNode gln, EntityManager em, Query q) {
        ...
        try {
            ...
            if (mainSbm != null && !getServiceManager().getSecurityManager()
            ...
            for (Submission s : res) {
                Path dir =
                        s == mainSbm ? getSubmissionPath(s) : getSubmissionHistoryPath(s);
        ...
        if (trnOk && getPublicFTPPath() != null && mainSbm != null) {
            Path ftpPath = getSubmissionPublicFTPPath(mainSbm);
    ...
    public Collection<Submission> searchSubmissions(User u, SubmissionSearchRequest ssr) throws ParseException {
        return queryHelper.searchSubmissions(u, ssr);
    }
    ...
    public LogNode updateSubmissionMeta(String acc, Collection<TagRef> tgRefs, Set<String> access, long rTime,
            User usr) {
        ErrorCounter ec = new ErrorCounterImpl();
        SimpleLogNode gln = new SimpleLogNode(SUCCESS, "Amending submission '" + acc + "' meta information", ec);
        ...
        try {
            ...
            if (!getServiceManager().getSecurityManager().mayUserUpdateSubmission(sbm, usr)) {
            ...
            if (access != null && access.size() > 0) {
                if (getSubscriptionEmailSubject() != null) {
                    wasPublic = getServiceManager().getSecurityManager().mayEveryoneReadSubmission(sbm);
                }
            ...
            if (access != null) {
                ...
                if (getSubscriptionEmailSubject() != null && sbm.getTagRefs() != null
                        && sbm.getTagRefs().size() > 0) {
                    nowPublic = getServiceManager().getSecurityManager().mayEveryoneReadSubmission(sbm);
                }
            }
        ...
        if (trnOk) {
            ...
            Path trnSbmPath = getSubmissionPath(sbm);
    ...
    private LogNode changeOwnerByAccession(String sbmAcc, String owner, User usr, boolean isLike) {
        ...
        if (newOwner == null) {
            newOwner = getServiceManager().getSecurityManager().getUserByEmail(owner);
        }
